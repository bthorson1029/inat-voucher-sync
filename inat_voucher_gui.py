...
def _crop_label_region(img):
    """
    Detect the white/light voucher label in a field photo, correct its
    perspective (handles angled/tilted placement), and return an upscaled
    grayscale crop ready for Tesseract.

    Uses percentile-based thresholding so detection adapts to ambient light —
    a shaded forest photo needs a much lower threshold (~140) than a bright
    daylight shot (~210).  Multiple percentile levels are tried in order from
    strict to lenient; the first one that yields a valid label-shaped region
    is used.
    """
    import cv2
    import numpy as np
    ...
    # Morphological kernel scales with image size.
    ksize  = max(5, min(w, h) // 80)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (ksize, ksize))

    # Build a de-duplicated list of threshold values from image percentiles.
    # Percentiles 92->83 span ~155->130 for a shaded photo and ~215->195
    # for a bright daylight shot — adapting correctly to both.
    raw_thresholds = [int(np.percentile(gray, p)) for p in (92, 90, 88, 86, 83)]
    seen, thresholds = set(), []
    ...
    for thresh_val in thresholds:
        ...
        for cnt in contours:
            ...
            # Label should be 0.3% – 25% of the full photo.
            if not (img_area * 0.003 <= area <= img_area * 0.25):
                continue
            ...
            # BT voucher labels are roughly 2:1 to 3.5:1.
            if 1.4 <= aspect <= 4.5 and area > best_area:
                best_area = area
                best_rect = rect

        if best_rect:
            break   # stop at the first threshold that found a candidate

    if best_rect is None:
        return None

    # Perspective-correct the tilted rectangle.
    box = cv2.boxPoints(best_rect).astype(np.float32)
    src = _order_points(box)
    rw  = int(max(best_rect[1]))
    rh  = int(min(best_rect[1]))
    dst = np.array([[0, 0], [rw - 1, 0],
                    [rw - 1, rh - 1], [0, rh - 1]], dtype=np.float32)
    M      = cv2.getPerspectiveTransform(src, dst)
    warped = cv2.warpPerspective(img, M, (rw, rh))

    # Upscale the crop to a consistent height for Tesseract.
    # Target 500 px tall; 3x floor keeps tiny labels legible.
...
def ocr_fallback(image_bytes, voucher_re, tesseract_cmd=None):
    ...
    # ------------------------------------------------------------------ #
    # Pass 1 — cropped, deskewed label region                             #
    # ------------------------------------------------------------------ #
    crop = _crop_label_region(img)
    if crop is not None:
